...
void initADC(void) {
    ...
    AD1CHSbits.CH0NA = 0; //leave negative input as V_REFL
    ...
    AD1CON1bits.SSRC = 0b111; //set so setting the SAMP starts sampling, and an internal counter ends sampling and starts conversion (if we set ASAM=1, which I have done SAMP is also started automatically)
    AD1CON3bits.SAMC = 0b11111; //this will determine how long we sample for currently. Set to 31 T_AD (ADC clock cycle, which is set equal to the internal clock cycle at the moment). This will not set time if SSRC is set for some timer
